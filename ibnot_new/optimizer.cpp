...
unsigned Scene::optimize_all_gradient_weights(FT wstep, FT xstep,
                                              unsigned max_weight_iters,
                                              FT epsilon, unsigned max_iters,
                                              std::ostream& out)
{
    bool global_connectivity = m_fixed_connectivity;
    ...
    while (iters < max_iters)
    {
        iters++;
        reset_weights();
        nb_assign += optimize_weights_via_gradient_descent_until_converge(wstep,
                                                                          coarse_wthreshold,
                                                                          0,
                                                                          max_weight_iters);
        FT norm = optimize_positions_via_lloyd(true);
    ...
    optimize_weights_via_gradient_descent_until_converge(wstep,
                                                         0.1 * fine_wthreshold,
                                                         0,
                                                         max_weight_iters);
