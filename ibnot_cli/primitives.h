...
public:
    ...
    const Bare_point& get_position() const 
    { 
        return this->point().point(); 
    }

    void set_position(const Bare_point& p)
    {
        FT w = get_weight();
    ...
    {
        const Bare_point& p = get_position();
        Weighted_point wp(p, w);
        this->set_point(wp);
    }
    ...
    Bare_point compute_centroid() const
    {
        FT sum_area = 0.0;
        ...
        for (unsigned i = 0; i < nb_pixels(); ++i)
        {
            ...
            Bare_point centroid = pixel.compute_centroid();
    ...
    {
        FT variance = 0.0;
        const Bare_point& q = get_position();
        for (unsigned i = 0; i < nb_pixels(); ++i)
        {
            const Pixel& pixel = get_pixel(i);
